...
market_cap_stocks = 'Stock Filtration/Data Collection/Market Cap Stocks.csv'  # Get List of Stocks
...
full_end_date = datetime.today()
full_start_date = full_end_date - relativedelta(months=6)
full_start_date_str = full_start_date.strftime('%Y-%m-%d')
full_end_date_str = full_end_date.strftime('%Y-%m-%d')

# Get Data to Calculate Trending ETFs
etf_data = {}
for etf in sector_etfs:
    data = yf.download(etf, start=full_start_date_str, end=full_end_date_str)
...
trending_file_path = 'Stock Filtration/Backtesting/ETF_Trending.csv'
...
plt.grid(True)
visualization_file_path = 'Stock Filtration/Backtesting/ETF_Trend_Visualization.png'
plt.savefig(visualization_file_path)
print(f"ETF Trend Visualization Saved at: {visualization_file_path}")

# Directories
base_data_dir = 'Stock Filtration/Backtesting'
...
def process_ticker(ticker_info):
    ...
    try:
        ...
        if stock_df is not None:  # Error Detection
            stock_df_path = f'Stock Filtration/Individual Stock Data/{selected_ticker}_Data.csv'  # Save to Folder
...
def main():
    ...
    lookback_start_date = final_end_date - relativedelta(months=6)
    lookback_start_date_str = lookback_start_date.strftime('%Y-%m-%d')
    final_end_date_str = final_end_date.strftime('%Y-%m-%d')
    tickers = df[ticker_column].unique()
    ticker_info_list = [(ticker, df, ticker_column, etf_column, sector_column, lookback_start_date_str, final_end_date_str) for ticker in tickers]  # Create List of Tuples
